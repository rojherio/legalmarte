...
$(document).ready(function() {
    ...
    } else if ($("#retorno_sucesso").val() == 3) {
        $("#msg_correcao").slideToggle().fadeOut(4000);
    } else if ($("#retorno_sucesso").val() == 5) {
    ...
    $('input[type="submit"]').livequery('click', function() {
        $.ajax({
            type: "POST",
            url: PORTAL_URL + 'dao/adm_fornecedor/analise',
            data: {status: $(this).attr('status'), codigo: $("#codigo").val()},
            cache: false,
            success: function(obj) {
                obj = JSON.parse(obj);
                onSuccessSendAnalise(obj);
            },
            error: function(obj) {
                onErrorAnalise(obj);
            }
    ...
    /* SUCESSO AO ENVIAR */
    function onSuccessSendAnalise(obj) {
        if (obj.msg == 'success') {
            postToURL(PORTAL_URL + 'adm_fornecedor/analise/' + obj.id, {retorno_sucesso: obj.retorno});
        }
        return false;
    }
    /* ERRO AO ENVIAR */
    function onErrorAnalise(args) {
        $("#msg_erro").slideToggle().fadeOut(4000);
        return false;
    }
    ...
    $(".checkbox").change(function() {
        ...
        var campo = $(this).parents('label.item').find("div").find("span.titulo").find('b').html();
        var campo2 = $(this).parents('label.item').find("div#resultado").find("span").html();

        if ($(this).find('input[type=checkbox]:checked').val() == 1) {//Marcado

            var antigo = $("#text_editor").html();

            if (antigo == "") {
                $("#text_editor").html(campo + " " + campo2);
            } else {
                $("#text_editor").html(antigo + "\n" + campo + " " + campo2);
            }

        } else {//Desmarcado
            $("#text_editor").html($("#text_editor").html().replace(campo + " " + campo2 + "\n", ''));
            $("#text_editor").html($("#text_editor").html().replace(campo + " " + campo2 + "", ''));
        }
