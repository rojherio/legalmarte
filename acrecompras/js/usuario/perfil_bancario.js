...
$('form#usuario_bancario').submit(function() {
    if (validar_formulario_bancario("")) {
        $.ajax({
            type: "POST",
            url: PORTAL_URL + 'dao/usuario/bancario',
            data: $('form#usuario_bancario').serialize(),
            cache: false,
            success: function(obj) {
                obj = JSON.parse(obj);
                onSuccessSendBancario(obj);
            },
            error: function(obj) {
                onErrorBancario(obj);
            }
...
function onSuccessSendBancario(obj) {
    if (obj.msg == 'success') {
        ...
        validar_formulario_bancario(obj);
    }
    return false;
}
/* ERRO AO ENVIAR */
function onErrorBancario(args) {
    $("#msg_erro").slideToggle().fadeOut(4000);
    return false;
}

//VALIDATOR FORMULÁRIO DE CADASTRO
function validar_formulario_bancario(obj) {
