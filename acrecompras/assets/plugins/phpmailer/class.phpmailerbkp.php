...
class PHPMailer {
   ...
   * Parameters:
   * bool $stmt result of the send action
  ...
  public function CreateHeader() {
    ...
    $stmt .= $this->HeaderLine('Date', self::RFCDate());
    if ($this->Sender == '') {
      $stmt .= $this->HeaderLine('Return-Path', trim($this->From));
    } else {
      $stmt .= $this->HeaderLine('Return-Path', trim($this->Sender));
    }
    ...
    if ($this->Mailer != 'mail') {
      if ($this->SingleTo === true) {
        ...
        if (count($this->to) > 0) {
          $stmt .= $this->AddrAppend('To', $this->to);
        } elseif (count($this->cc) == 0) {
          $stmt .= $this->HeaderLine('To', 'undisclosed-recipients:;');
        }
      }
    }
    ...
    $stmt .= $this->AddrAppend('From', $from);
    
    // sendmail and mail() extract Cc from the header before sending
    if (count($this->cc) > 0) {
      $stmt .= $this->AddrAppend('Cc', $this->cc);
    }
    
    // sendmail and mail() extract Bcc from the header before sending
    if ((($this->Mailer == 'sendmail') || ($this->Mailer == 'mail')) && (count($this->bcc) > 0)) {
      $stmt .= $this->AddrAppend('Bcc', $this->bcc);
    }
    
    if (count($this->ReplyTo) > 0) {
      $stmt .= $this->AddrAppend('Reply-to', $this->ReplyTo);
    }
    
    // mail() sets the subject itself
    if ($this->Mailer != 'mail') {
      $stmt .= $this->HeaderLine('Subject', $this->EncodeHeader($this->SecureHeader($this->Subject)));
    }
    
    if ($this->MessageID != '') {
      $stmt .= $this->HeaderLine('Message-ID', $this->MessageID);
    } else {
      $stmt .= sprintf("Message-ID: <%s@%s>%s", $uniq_id, $this->ServerHostname(), $this->LE);
    }
    $stmt .= $this->HeaderLine('X-Priority', $this->Priority);
    $stmt .= $this->HeaderLine('X-Mailer', 'PHPMailer ' . $this->Version . ' (phpmailer.sourceforge.net)');
    
    if ($this->ConfirmReadingTo != '') {
      $stmt .= $this->HeaderLine('Disposition-Notification-To', '<' . trim($this->ConfirmReadingTo) . '>');
    }
    
    // Add custom headers
    for ($index = 0; $index < count($this->CustomHeader); $index ++) {
      $stmt .= $this->HeaderLine(trim($this->CustomHeader[$index][0]), $this->EncodeHeader(trim($this->CustomHeader[$index][1])));
    }
    if (! $this->sign_key_file) {
      $stmt .= $this->HeaderLine('MIME-Version', '1.0');
      $stmt .= $this->GetMailMIME();
    }
  ...
  public function GetMailMIME() {
    ...
    switch ($this->message_type) {
      case 'plain' :
        $stmt .= $this->HeaderLine('Content-Transfer-Encoding', $this->Encoding);
        $stmt .= sprintf("Content-Type: %s; charset=\"%s\"", $this->ContentType, $this->CharSet);
        break;
      case 'attachments' :
      case 'alt_attachments' :
        if ($this->InlineImageExists()) {
          $stmt .= sprintf("Content-Type: %s;%s\ttype=\"text/html\";%s\tboundary=\"%s\"%s", 'multipart/related', $this->LE, $this->LE, $this->boundary[1], $this->LE);
        } else {
          $stmt .= $this->HeaderLine('Content-Type', 'multipart/mixed;');
          $stmt .= $this->TextLine("\tboundary=\"" . $this->boundary[1] . '"');
        }
        break;
      case 'alt' :
        $stmt .= $this->HeaderLine('Content-Type', 'multipart/alternative;');
        $stmt .= $this->TextLine("\tboundary=\"" . $this->boundary[1] . '"');
        break;
    }
    
    if ($this->Mailer != 'mail') {
      $stmt .= $this->LE . $this->LE;
    }
  ...
  private function GetBoundary($boundary, $charSet, $contentType, $encoding) {
    ...
    $stmt .= $this->TextLine('--' . $boundary);
    $stmt .= sprintf("Content-Type: %s; charset = \"%s\"", $contentType, $charSet);
    $stmt .= $this->LE;
    $stmt .= $this->HeaderLine('Content-Transfer-Encoding', $encoding);
    $stmt .= $this->LE;
  ...
  public static function RFCDate() {
    ...
    $stmt = sprintf("%s %s%04d", date('D, j M Y H:i:s'), $tzs, $tz);
  ...
  private function ServerHostname() {
    if (! empty($this->Hostname)) {
      ...
      $stmt = $_SERVER['SERVER_NAME'];
    } else {
      $stmt = 'localhost.localdomain';
    }
